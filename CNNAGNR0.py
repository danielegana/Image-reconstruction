...
#%%
lendata=200
R0 = np.random.randint(6, 100, size=(lendata,))
os.chdir(binarydir)
...
for x in directorylist:
    ...
    #%%
#%%
for x,y in enumerate(R0):
    with open(filedir+"inputs/input" + str(x), 'w') as file:
    # Write each number to the file
           file.write(str(R0[x]/np.max(R0)) + '\n')
           print(R0[x])
           bashcommand="./ssdisk"+" -run "+str(x)+" -R0 "+str(R0[x]) +" -Rmax "+str(150)
...
y_pred = np.ones(len(test_dataset))*0.5
...
plt.imshow(agndatavis[0][0][0:100,0:100])
plt.figure(figsize=(20, 15))
