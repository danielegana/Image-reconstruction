...
numparams=2
numpoints=5
lendata=pow(numpoints,numparams)
inclination = np.random.rand(numpoints)*np.pi/2
R0 = np.random.randint(6, 100, size=(numpoints,))

paramtuples = list(itertools.product(inclination, R0))
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        printout = pool.starmap(testpool.testfun, paramtuples)
...
for x,(inclinationit,R0it) in enumerate(paramtuples):
    ...
    # Write each number to the file
           ...
           bashcommand="./ssdisk"+" -run "+str(x)+" -i "+str(inclinationit)+"-R0 "+str(R0it)
